            ...
            context("can sort its data") {
                ...
                it("after the most starred") {
                    ...
                    for (index, higherStarCountRep) in viewModel.sortedAndFilterdData.value.enumerated() {
                        for (secondIndex, lowerStarCountRep) in viewModel.sortedAndFilterdData.value.enumerated()
                            where secondIndex > index {
                            expect(higherStarCountRep.numberOfStars)
                                .to(beGreaterThanOrEqualTo(lowerStarCountRep.numberOfStars))
                        }
                    }
                }
                it("after the most Watched") {
                    ...
                    for (index, higherWatchedCountRep) in viewModel.sortedAndFilterdData.value.enumerated() {
                        for (secondIndex, lowerWatcherCountRep) in viewModel.sortedAndFilterdData.value.enumerated()
                            where secondIndex > index {
                            expect(higherWatchedCountRep.numberOfWatchers)
                                .to(beGreaterThanOrEqualTo(lowerWatcherCountRep.numberOfWatchers))
                        }
                    }
                }
                it("after the most Forked") {
                    ...
                    for (index, higherForkedCountRep) in viewModel.sortedAndFilterdData.value.enumerated() {
                        for (secondIndex, lowerForkedCountRep) in viewModel.sortedAndFilterdData.value.enumerated()
                            where secondIndex > index {
                            expect(higherForkedCountRep.numberOfForks)
                                .to(beGreaterThanOrEqualTo(lowerForkedCountRep.numberOfForks))
                        }
                    }
                }
